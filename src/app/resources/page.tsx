...
export default async function ResourcesPage() {
  let categories: { title: string; icon: React.ReactNode; color: string; items: string[] }[] = [];

  try {
    await dbConnect();
    const resources = await Resource.find({ published: true }).sort({ createdAt: -1 }).lean();
    ...
    categories = Object.entries(grouped).map(([title, items]) => ({
      title,
      icon: categoryConfig[title]?.icon || categoryConfig["Mental Health"].icon,
      color: categoryConfig[title]?.color || "#4a9e6e",
      items,
    }));
  } catch (e) {
    console.error("Failed to load resources from MongoDB:", e);
  }
      ...
      <section className="px-6 py-24">
        <div className="mx-auto max-w-7xl">
          {categories.length > 0 ? (
            ...
            <p className="text-center text-zinc-500">No resources available yet.</p>
          )}
        </div>
      </section>
